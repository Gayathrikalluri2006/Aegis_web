...
import { computeSafetyScore, mockFactorsFor, scoreLabel, fetchRealBase, type SafetyFactors } from "@/lib/safety";
...
function timeBucket() { return Math.floor(Date.now() / (5 * 60 * 1000)); }
...
function NavigatePage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const updated = await Promise.all(routes.map(async (r) => {
  ...
  useEffect(() => {
    ...
    const load = () => {
      supabase.from("incident_reports").select("id,lat,lng,type,severity").limit(500).then(({ data }) => {
        if (active && data) setIncidents(data.map((d) => ({ id: d.id, lat: d.lat, lng: d.lng, type: d.type, severity: d.severity })));
      });
    };
    load();
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    try {
      ...
      const bucket = timeBucket();
      const hour = new Date().getHours();
      const scored: ScoredRoute[] = await Promise.all(paths.slice(0, 3).map(async (p, i) => {
