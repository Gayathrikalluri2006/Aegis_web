...
export function fetchRealBase(lat: number, lng: number): Promise<RealBase> {
  ...
  const p = (async (): Promise<RealBase> => {
    ...
    try {
      ...
      clearTimeout(to);
      if (res.ok) {
        const j = await res.json() as { current?: { precipitation?: number; wind_speed_10m?: number; weather_code?: number; temperature_2m?: number } };
        const c = j.current ?? {};
        ...
        const tempRisk = c.temperature_2m != null ? Math.min(20, Math.max(0, Math.abs(c.temperature_2m - 18) - 8)) : 0;
    ...
    const base: RealBase = {
      ...
      crowdDensity: Math.round(clamp(30 + rPlace() * 30 + (isRushHour ? 25 : 0) + (isNight ? -15 : 0) + rTime() * 15)),
